...
Os botões de "instalar no Android" (PLAY_STORE_URL), "apoiar" (APOIO_URL),
"blog" (BLOG_URL) e "instagram" (INSTAGRAM_URL) só aparecem quando a env var
correspondente está preenchida — assim o menu/página fica coerente antes de a
conta/destino existir.
"""
...
.lp-prose p{margin-top:12px}.lp-prose p:first-child{margin-top:0}
...
def render_landing(
    ...
    # menu do topo — âncoras internas + destinos externos condicionais
    nav_links = ['<a href="#quem-somos">quem somos</a>']
    if blog_url:
        nav_links.append(f'<a href="{_esc(blog_url)}" rel="noopener" target="_blank">blog</a>')
    nav_links.append('<a href="#contribua">contribua</a>')
    nav_links.append(f'<a class="lp-nav-cta" href="{_esc(app_url)}">abrir o app</a>')
    nav = (
        '<nav class="lp-nav">'
        f'<a class="lp-brand" href="#top">{_logo("")}Lombada<span class="dot">.</span></a>'
        f'<div class="lp-nav-links">{"".join(nav_links)}</div>'
        '</nav>'
    )
    ...
    # os dois pilares logo abaixo do hero
    pillars = "".join([
    ...
    # ── quem somos ──
    quem_somos = (
        '<section class="lp-section" id="quem-somos"><span class="label">quem somos</span>'
        '<h2>Um projeto independente, feito por quem lê</h2>'
        '<div class="lp-prose">'
        ...
        '<p>É um projeto independente, tocado por uma pessoa só, com catálogo '
        'próprio alimentado por raspagem direta dos sites das editoras brasileiras. '
        'Sem anúncio e sem venda de dado: seus livros são seus.</p>'
        '</div></section>'
    )
    ...
    # rodapé
    foot_links = [f'<a href="{_esc(app_url)}">abrir o app</a>', '<a href="#quem-somos">quem somos</a>']
    if blog_url:
        foot_links.append(f'<a href="{_esc(blog_url)}" rel="noopener" target="_blank">blog</a>')
    foot_links.append('<a href="#contribua">contribua</a>')
    if instagram_url:
    ...
    og_tags += '<meta name="description" content="Rede social de livros e a fonte mais completa pra pesquisar livros publicados no Brasil.">'
    ...
    return (
        ...
        '}catch(e){}})()</script>'
        '<title>Lombada — rede social de livros e catálogo do Brasil</title>'
